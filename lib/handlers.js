...
// Handler para obtener el SEO de un video (v7.8)
handlers.apiSeo = function(data, callback) {
  // Aceptar solo el método GET
  if (data.method == 'get') {
    // Obtener el ID del video del query string
    var videoId = typeof(data.queryStringObject.id) == 'string' && data.queryStringObject.id.trim().length > 0 ? data.queryStringObject.id.trim() : false;

    if (videoId) {
      // Llamar a la nueva función en lib/youtube.js
      youtube.getVideoSeoDetails(videoId, function(err, seoData) {
        if (!err && seoData) {
          callback(200, seoData);
        } else {
          callback(500, { 'Error': 'No se pudo obtener la información del video: ' + err });
        }
      });
    } else {
      callback(400, { 'Error': 'Falta el parámetro ID del video' });
    }
  } else {
    callback(405, { 'Error': 'Método no permitido' });
  }
};

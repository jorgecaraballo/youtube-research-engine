...
// Parse querystring to an object in all cases, without throwing
helpers.parseQueryStringToObject = function(str) {
	var objeto = {};
	if (str == '') {
		return objeto;
		}
		...
		try {
			...
			objeto = JSON.parse('{"' + decodeURI(str).replace(/"/g, '\\"').replace(/&/g, '","').replace(/=/g,'":"') + '"}');
			return objeto;
			}
		catch (e) {
			return objeto;
...
helpers.sendTwilioSms = function(phone,msg,callback) {
	...
	if (phone && msg) {
		// Configure the request payload that we're gonna send to Twilio
		var payload = {
...
// Get the contents of a static (socket.io/js) asset
helpers.getStaticAssetSocketIoJs = function(fileName,callback) {
	fileName = typeof(fileName) == 'string' && fileName.length > 0 ? fileName : false;
	if (fileName) {
		var publicDir = path.join(__dirname,'/../node_modules/socket.io/client-dist/');
		fs.readFile(publicDir+fileName,function(err,data) {
			if (!err && data) {
				callback(false,data);
				}
